        ...
        try {
            ...
            notices.forEach(function (notice) {
                ...
                card.innerHTML = [
                    ...
                    '<button class="mini-btn edit-btn" type="button" data-notice-json="' + encodeURIComponent(JSON.stringify(notice)) + '">Edit</button>',
    ...
    function bindNoticeActions() {
        document.querySelectorAll(".edit-btn").forEach(function (button) {
            button.addEventListener("click", function () {
                const notice = JSON.parse(decodeURIComponent(button.dataset.noticeJson));
                populateForm(notice);
            });
        });
        ...
        document.querySelectorAll(".delete-btn").forEach(function (button) {
            button.addEventListener("click", async function () {
                try {
                    clearFeedback();
                    await fetchJson(API_BASE_URL + "/" + button.dataset.noticeId, {
                        method: "DELETE"
                    });
                    if (document.getElementById("editingNoticeId").value === button.dataset.noticeId) {
                        resetForm();
                    }
        ...
        document.querySelectorAll(".toggle-status-btn").forEach(function (button) {
            button.addEventListener("click", async function () {
                ...
                try {
                    clearFeedback();
                    await fetchJson(API_BASE_URL + "/" + button.dataset.noticeId + "/status?active=" + (!currentState), {
                        method: "PATCH"
                    });
